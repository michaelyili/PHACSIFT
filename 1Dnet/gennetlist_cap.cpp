...
int main(int argc, char* argv[])
{
	...
	fprintf(fp, ".lib '/home/liyi/65n_lib/l0065ll_v1p5.lib' tt\n\n");
	...
	fprintf(fp, "\n** capacity c\n");
	for (int i=0; i < size; i++) {
		fprintf(fp, "c%d %d %d 10f\n", i+1, (i+1)+2*size, i+1+size);
	}
	...
	fprintf(fp, "\n** resistor r2\n");
	for (int i=0; i < size-2; i++) {//-1k\n", // test the difference between spectre and Hspice
	...
	fprintf(fp, "\n");
	fprintf(fp, "\n.op\n");
	fprintf(fp, ".tran 100ns 2s uic\n");
	//fprintf(fp, ".dc v%d 1v 2v 1v\n", (size+1)/2);
	fprintf(fp, ".print ");
	//for (int i=0; i < size; i++) {
	//	fprintf(fp, " v(%d)", (i+1) + size);
	//}
	fprintf(fp, " v(%d)", size/2 - 2 + 2*size);
	fprintf(fp, " v(%d)", size/2 - 1 + 2*size);
	fprintf(fp, " v(%d)", size/2 + 2*size);
	fprintf(fp, " v(%d)", size/2 + 1 + 2*size);
	fprintf(fp, " v(%d)", size/2 + 2 + 2*size);
	fprintf(fp, " v(%d)", size/2 + 3 + 2*size);
	fprintf(fp, " v(%d)", size/2 + 4 + 2*size);
